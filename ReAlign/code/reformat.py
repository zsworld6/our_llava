...
from util import ChatGPT_API, get_image
import time
...
def rewrite_response_request(image, question, response, structure, evidences, args):
    if len(evidences) > 0:
    ...
    rewrited_response = ChatGPT_API(system_instruction, user_input, args.model, args.openai_key,
...
def main():
    ...
    # OPENAI_API_BASE=https://api3.apifans.com/v1
    ...
    for data in tqdm(process_data):
        ...
        if "image" in data and data["image"] != None and data["image"] != "":
            image = "/data2/cs2916_t1/ScienceQA/data/scienceqa/images/train/" + data["image"]
        else:
            image = None
        ...
        while True:
            try:
                new_response_list = rewrite_response_request(image, question, response, structure, evidences, args)
